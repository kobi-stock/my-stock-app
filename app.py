...
def load_sheet_data(gid):
    try:
        ...
        # 종목코드가 숫자로 인식되어 앞의 0이 잘리는 것을 방지하기 위해 dtype=str 사용
        df = pd.read_csv(url, dtype={'코드': str})
        return df
    except Exception as e:
        st.error(f"⚠️ [{gid}] 탭 로드 실패: 구글 시트의 '공유' 설정을 다시 확인해 주세요. (에러: {e})")
        return pd.DataFrame()
...
if df.empty:
    st.warning("데이터가 없습니다. 구글 시트 내용이나 공유 설정을 확인해 주세요.")
    st.stop()
...
if selected_account == "전체 계좌":
    ...
    st.info(f"💡 전체 계좌 합산 예수금: {cash:,}원 (각 계좌 화면에서 수정 가능)")
else:
...
@st.cache_data(ttl=10)
def get_price(code):
    if not code or code == "nan": return 0
    try:
        # 코드가 6자리가 되도록 앞을 0으로 채움
...
if active_stocks:
    ...
    st.info("현재 보유 중인 종목이 없습니다. 구글 시트의 데이터를 확인해 주세요.")