...
# 💹 2. 실시간 시세 엔진 (수동 입력 없이 즉시 반영)
@st.cache_data(ttl=5)
def get_live_price(code):
    clean_code = re.sub(r'[^0-9]', '', str(code)).zfill(6)
    try:
        # 네이버 실시간 API 사용하여 즉시 시세 획득
...
if active_stocks:
    ...
    for name in active_stocks:
        d = portfolio[name]
        curr_p = get_live_price(d["code"]) # 실시간 주가 즉시 적용
    ...
    row1_3.metric("💵 총수익", f"{int(total_profit_amt):+,}원", f"{total_profit_rate:+.2f}%")

    row2_1, row2_2, row2_3 = st.columns(3)
    row2_1.metric("📈 총평가액", f"{int(total_eval):,}원")
    row2_2.metric("🏦 총자산", f"{int(total_asset):,}원")
    row2_3.metric("📊 수익률", f"{total_profit_rate:+.2f}%")
    st.divider()
