    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding='utf-8') as f:
                data = json.load(f)
                for key in ["cash", "history"]:
                    if key not in data: data[key] = {}
                return data
        except: return {"cash": {}, "history": {}}
    return {"cash": {}, "history": {}}
...
real_time_total_asset = total_cash + total_eval # 합산값
...
# 전일 대비 계산 (현재 합산값 - 어제 시트 기록값)
yesterday_val = get_past_val(1)
d_diff = real_time_total_asset - yesterday_val if yesterday_val > 0 else 0
d_rate = (d_diff / yesterday_val * 100) if yesterday_val > 0 else 0

summary_html = '<div class="card-container">'
# 1행: 변동
cls = "up" if d_diff > 0 else "down" if d_diff < 0 else ""
summary_html += f'<div class="custom-card"><div class="card-label">전일대비</div><div class="card-value">{int(d_diff):+,}</div><div class="card-delta {cls}">{d_rate:+.2f}%</div></div>'
# 2행: 실시간 요약
...
summary_html += f'<div class="custom-card"><div class="card-label">🏦 총잔고</div><div class="card-value">{int(real_time_total_asset):,}</div></div>'
...
for n in active_stocks:
    ...
    res_data.append([n, portfolio[n]["qty"], int(ba), cp, int(ev), ((cp-ba)/ba*100), (ev/real_time_total_asset*100)])

res_data.append(["💰 예수금", None, None, None, int(total_cash), None, (total_cash/real_time_total_asset*100)])
